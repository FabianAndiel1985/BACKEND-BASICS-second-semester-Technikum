...
class ShowDataController {
	...
	public function route () {
		...
		if($this->queryBuilderModel != "") {
			...
			$this->buildView($queryResultArray);
	...
	private function getAndCheckURLParameters() {
		...
		switch ($_GET['action']) {
		    ...
		    default:
		        echo "Please enter an valid action type and/or a type id";
		}
	}
	...
	private function buildView($queryResultArray) {
			if($this->typeId == "") {
				$this->jsonView->buildListTypesView($queryResultArray);
			}

			else {
			$this->jsonView->buildListProductsByTypeIdView($queryResultArray);
			}
	}
}